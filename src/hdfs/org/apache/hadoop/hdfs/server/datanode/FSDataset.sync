 ...
 *
 ***************************************************/
//-----replace by wanglei 2011.11.30.01
public class FSDataset implements FSConstants, FSDatasetInterface {
  ...
  class FSVolume {
    ...
    FSVolume(File currentDir, Configuration conf) throws IOException {
      //this.reserved = conf.getLong("dfs.datanode.du.reserved", 0);update by wzt 2012-12-25
      ...
      this.reserved = this.usage.getReserved();//add by wzt 2012-12-25
    ...
    long getAvailable() throws IOException {
      ...
      DataNode.LOG.info("wzt debuginfo:FSVolume remaining="+remaining+" available="+available+"");
  ...
  static class FSVolumeSet {
    ...
    synchronized FSVolume getNextVolume(long blockSize) throws IOException {
      ...
      while (true) {
        ...
        //if (volume.getAvailable() > blockSize) { return volume; } //update by wzt 2012-12-20
        long availableVolumeSize = volume.getAvailable();
        if (availableVolumeSize > blockSize) { return volume; }
        if (curVolume == startVolume) {
         //throw new DiskOutOfSpaceException("Insufficient space for an additional block");//update by wzt 2012-12-20
       	 throw new DiskOutOfSpaceException(
        	"Insufficient space for an additional block. Available volume size is "
        	+ availableVolumeSize + ",  Configured block size is "+blockSize);
        }
      }
    }
    ...
    long getDfsUsed() throws IOException {
      ...
      for (int idx = 0; idx < volumes.length; idx++) {
        //dfsUsed += volumes[idx].getDfsUsed();//update by wzt 2012-12-20
    	 //add by 2012-12-20
      	long volumesDfsUsed = volumes[idx].getDfsUsed();  
    	dfsUsed += volumesDfsUsed;
        DataNode.LOG.info("wzt debuginfo:FSDataset volumes["+idx+"] volumesDfsUsed="+volumesDfsUsed);
        //end add
      }
      return dfsUsed;
    }
    ...
    synchronized long getCapacity() throws IOException {
      long capacity = 0L;
      for (int idx = 0; idx < volumes.length; idx++) {
        //capacity += volumes[idx].getCapacity();//update by wzt 2012-12-20
    	//add by 2012-12-20
    	long volumesCapacity = volumes[idx].getCapacity();
    	capacity += volumesCapacity;
        DataNode.LOG.info("wzt debuginfo:FSDataset volumes["+idx+"] volumesCapacity="+volumesCapacity);
    	//end add
      }
      return capacity;
    }
    ...
    synchronized long getRemaining() throws IOException {
      long remaining = 0L;
      for (int idx = 0; idx < volumes.length; idx++) {
        //remaining += volumes[idx].getAvailable();//update by wzt 2012-12-20
    	  //add by 2012-12-20
    	long volumeRemaining = volumes[idx].getAvailable();
        remaining += volumeRemaining;
        DataNode.LOG.info("wzt debuginfo:FSDataset volumes["+idx+"] volumeRemaining="+volumeRemaining);
        //end add
      }
      return remaining;
    }

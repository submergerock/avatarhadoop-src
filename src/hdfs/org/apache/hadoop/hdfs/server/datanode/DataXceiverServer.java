...
class DataXceiverServer implements Runnable, FSConstants {
  ...
  public void run() {
    while (datanode.shouldRun) {
      Socket s = null;//add by wzt 2012-12-09
      try {
    	s = ss.accept();//Socket s = ss.accept();update by wzt 2012-12-09
      ...
      }catch (AsynchronousCloseException ace){//add by wzt 2012-12-09
          // another thread closed our listener socket - that's expected during
          // shutdown,
          // but not in other circumstances
          if (datanode.shouldRun) {
            LOG.info("wzt debuginfo:"+datanode.dnRegistration + ":DataXceiverServer: ", ace);
          }
      }catch (IOException ie) {
    	IOUtils.closeSocket(s);//add by wzt 2012-12-09  
        LOG.info("wzt debuginfo:close socket "+datanode.dnRegistration + ":DataXceiveServer: " 
                                + StringUtils.stringifyException(ie));
      }catch (OutOfMemoryError ie){//add by wzt 2012-12-09
    	IOUtils.closeSocket(s);
    	// DataNode can run out of memory if there is too many transfers.
    	// Log the event, Sleep for 30 seconds, other transfers may complete by
        // then.
    	LOG.info("wzt debuginfo:DataNode is outofmemory. Will retry in 30 seconds.", ie);
    	try {
    ...
    try {
      ...
      LOG.info("wzt debuginfo:accept socket close exception "+datanode.dnRegistration + ":DataXceiveServer: " 
                              + StringUtils.stringifyException(ie));
    }
  }

...
//  }
    ...
    Daemon d = new Daemon(threadGroup, new Runnable() {
      ...
      public void run() {
        for(int i = 0; i < blocks.length; i++) {
          ...
          } catch (IOException e) {//update by wzt 2013-12.17
            LOG.warn("recoverBlocks FAILED, src blocks[" + i + "]=" + blocks[i], e);
          }
        }
      }
    });
  ...
  private LocatedBlock syncBlock(Block block, List<BlockRecord> syncList,
    ...
    //add by wzt 2013-12.18
    LOG.info("block=" + block + ", (length=" + block.getNumBytes()
            + "), syncList=" + syncList + ", closeFile=" + closeFile);
    //end by
    ...
    //异常集中在这里，由于updateBlock失败导致抛出了异常

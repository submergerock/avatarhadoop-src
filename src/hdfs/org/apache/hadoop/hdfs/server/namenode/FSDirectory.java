...
public class FSDirectory implements FSConstants, Closeable {
  ...
  //add by wzt 2013-0129
  //lock to protected meta tree
  ...
  Block addBlock(String path, INode[] inodes, Block block) throws IOException {
    ...
    try{
          ...
          NameNode.stateChangeLog.debug("DIR* FSDirectory.addFile: "
                                        ...
                                        + " block is added to the in-memory "
                                        + "file system");
          //add by wzt 2013-0403
          NameNode.stateChangeLog.info("wzt testblockmap DIR* FSDirectory.addFile: "
                  + path + " with " + block
                  + " block is added to the in-memory "
                  + "file system");
  ...
  void closeFile(String path, INodeFile file) throws IOException {
    ...
    try{
          ...
          NameNode.stateChangeLog.info("DIR* FSDirectory.closeFile: "
                  +path+" with "+ file.getBlocks().length 
                  +" blocks is persisted to the file system");
          //end add
    }finally{
  ...
  INode unprotectedMkdir(String src, PermissionStatus permissions,
    ...
    INode[] inodes = new INode[components.length];
    NameNode.stateChangeLog.info("wzt testinfo:unprotectedMkdir components="+components.length+" inodes="+inodes.length);
    //update by wzt 2013
    //synchronized (rootDir) {
    //  rootDir.getExistingPathINodes(components, inodes);
    ...
    try{
    	...
    	NameNode.stateChangeLog.error("unprotectedMkdir", er);
    }
    finally{
    	this.writeUnLock();
    }

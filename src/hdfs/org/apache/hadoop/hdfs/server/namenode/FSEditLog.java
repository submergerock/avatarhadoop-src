  ...
  public synchronized void open() throws IOException {
    ...
    for (Iterator<StorageDirectory> it = 
      ...
      try {
        ...
        Log.info("open editFile : " + eFile.getPath());
	  ...
	  while (isSyncRunning) {
	    ...
	    for (int idx = 0; idx < editStreams.size(); idx++) {
	      ...
	      try {
	        ...
	        eStream.flush();
	        eStream.close();
	        Log.info("close editFile : " + eStream.getName());
	      } catch (IOException e) {
	        processIOError(idx);
	        idx--;
	      }
...
//	    }
  ...
  public void logSync() throws IOException {
    ...
    synchronized (this) {
      ...
      // swap buffers
      for (int idx = 0; idx < editStreams.size(); idx++) {//---------------
    	  if(idx >= editStreams.size()) break;//added by wanglei 2012.1.6
  ...
  synchronized long getFsEditTime() {
    ...
    if(it.hasNext()){
    	StorageDirectory sd = it.next();
    	FSNamesystem.LOG.info("editLog : " + getEditFile(sd).getPath());
    	FSNamesystem.LOG.info("editLog.lastModifiedTime : " + getEditFile(sd).lastModified());
      return getEditFile(sd).lastModified();
    }
    return 0;
  }

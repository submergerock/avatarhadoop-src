...
public class NamenodeFsck {
  ...
  public void fsck() throws IOException {
	//add by wzt 2013-0327
    LOG.info("wzt testinfo: log.info fsck");
	out.println("wzt testinfo: out.print fsck");
	//end add
	  
    try {
      FileStatus[] files = nn.namesystem.dir.getListing(path);
      //add by wzt 2013-0327
      out.println("wzt testinfo:fsck path="+path);
      //end add
      ...
      if (files != null) {
        for (int i = 0; i < files.length; i++) {
        	//add by wzt testinfo
        	out.println("wzt testinfo:fsck file="+files[i].getPath()+" size="+files[i].getBlockSize() + " len="+files[i].getLen()
  ...
  private void check(FileStatus file, FsckResult res) throws IOException {
    ...
    out.println("wzt testinfo:check minReplication="+minReplication+" path="+path+" file.isDir="+file.isDir());
    //end add
    if (file.isDir()) {
      FileStatus[] files = nn.namesystem.dir.getListing(path);
      //add by wzt 2013-0326
      out.println("wzt testinfo:check path="+path);
      //end add
      
      if (files == null) {
      	//add by wzt 2013-0327
      	out.println("wzt testinfo:files == null");
      	//end add
    ...
    //add by wzt 2013-0328
    out.println("wzt testinfo: 1 filelen="+fileLen+" getblocklocation path="+path);
    if(nn == null){
    	out.println("nn == null");
    }else{
    	if(nn.namesystem == null){
    		out.println("nn.namesystem == null");
    	}else{
    		out.println("nn.namesystem.getBlockLocation is used");
    	}
    ...
    if(blocks == null){
        //add by wzt 2013-0327
        out.println("wzt testinfo:check blocks == null");	
        //end add
    	
    }else{
        out.println("wzt testinfo: check fileLen="+fileLen+" blocks="+blocks.getFileLength()+" blockcount="+blocks.locatedBlockCount());    	
    }
    //end add
    ...
    if (isOpen && !showOpenFiles) {
      ...
      res.totalOpenFiles++;
      //add by wzt 2013-0327
      out.println("wzt testinfo:check isOpen && !showOpenFiles openblocks="+res.totalOpenFilesBlocks+" openfiles="+res.totalOpenFiles);
      //end add
      return;
    }
    res.totalFiles++;
    ...
    //add by wzt 2013-0328
    out.println("wzt testinfo:check totalFiles="+res.totalBlocks+" totalsize="+res.totalSize+" totalblocks="+res.totalBlocks);
    //end add
    ...
    int i = 0;
    //add by wzt 2013-0327
    out.println("wzt testinfo:blocks size="+blocks.getLocatedBlocks().size());
    //end add
    ...
    for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
      ...
      //add by wzt 2013-0327
      out.println("wzt testinfo:check missingRacks="+missingRacks);
      //end add
    ...
    //add by wzt 2013-0327
    out.println("wzt testinfo:check end file...");
    //end add
  ...
  private void lostFoundMove(FileStatus file, LocatedBlocks blocks)
    throws IOException {
	//add by wzt 2013-0327
	 LOG.info("wzt testinfo:log.info lostFoundMove");
	 out.println("wzt testinfo:out.println lostFoundMove");
	//end add
  ...
  private void copyBlock(DFSClient dfs, LocatedBlock lblock,
	...
	//add by wzt 2013-0327
	LOG.info("wzt testinfo:log.info copyBlock");
	out.println("wzt testinfo:out.println copyBlock");
	//end add

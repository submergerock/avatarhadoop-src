...
import com.dinglicom.com.hdfs.FastLog;
...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  private static final void logAuditEvent(UserGroupInformation ugi,
	  ...
	  builder.append("ugi=").append(ugi).append("\t")
	          ...
	          fastLogthread.addAuditLog(builder.toString());
  }
  public static FastLog fastLogthread = null;
  //end add
...
//add by wl826214---------2012.2.16---end
  //add by wzt 2013-0225
  private float  replicationWorkMultiplier;
  //end add
  //add by wzt 2013-0205
  boolean hasRwLock = false;//share we use read/write locks?
  private ReentrantReadWriteLock fsLock;
  //utility methods to acquire and release read lcok and write lcok
  ...
  private void initialize(NameNode nn, Configuration conf) throws IOException {
	//add by wzt 2013-0312
	//LOG.debug("wzt rtlock:FSNamesystem initialize ... ");
	//end add
	//add by wl826214---------2012.2.16---start
	this.conf = conf;
	//add by wl826214---------2012.2.16---end
    ...
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:dir.loadFSImage...");
    //end add
    ...
    setBlockTotal();
    //add by wzt 2013-0311
    //LOG.debug("wzt rtlock:new PendingReplicationBlocks...");
    //end add
    ...
    lmthread.start();
    //add by wzt 2013-0311
    //LOG.debug("wzt rtlock:hbthread,lmthreadstart");
    //end add
    replthread.start();
    //add by wzt 2013-0311
    //LOG.debug("wzt rtlock:lmthread  start");
    //end add
  ...
  private void setConfigurationParameters(Configuration conf) 
    ...
    //add by wzt 2013-0205
    this.hasRwLock = conf.getBoolean("dfs.rwlock", true);
    this.replicationWorkMultiplier = ReplicationMonitor.REPLICATION_WORK_MULTIPLIER_PER_ITERATION *
            conf.getFloat("dfs.replication.iteration.multiplier",10);
    //end add
  ...
  NamespaceInfo getNamespaceInfo() {
	//update by wzt 2013-0205
    //return new NamespaceInfo(dir.fsImage.getNamespaceID(),
    //                         dir.fsImage.getCTime(),
    //                         getDistributedUpgradeVersion());
	this.writeLock();
	try{
		return new NamespaceInfo(this.dir.fsImage.getNamespaceID(),
				                 this.dir.fsImage.getCTime(),
				                 getDistributedUpgradeVersion());
	}finally{
		this.writeUnlock();
	}
  ...
  public void setPermission(String src, FsPermission permission
	...
	try{
		...
		dir.setPermission(src, permission);
	      if (auditLog.isInfoEnabled() ) {
	          resultingStat = dir.getFileInfo(src);
	        }
	}finally{
		this.writeUnlock();
	}
  ...
  public LocatedBlocks getBlockLocations(String src, long offset, long length,
    ...
    //add by wzt 2013-0206
    if(FSNamesystem.LOG.isDebugEnabled()) {
    	FSNamesystem.LOG.debug("filePath : " + src);    	
    }
    //end add
    //add by wzt 2013-0328
    //FSNamesystem.LOG.info("wzt debugfsck: getBlockLocations --> getBlockLocationsInternal src="+src+
    //		" offset="+offset+" length="+length+" doAccessTime="+doAccessTime);
    //end add
  ...
  private  LocatedBlocks getBlockLocationsInternal(String src,
	  ...
	  for(int attempt = 0;attempt < 2;attempt++){
		  if(attempt == 0){//first attempt is with readlock
			 ...
			 this.writeLock(); //writelock is needed to set accesstime
			 //we need to refetch the node in case it was deleted
			 //while the lock was not held
		  }
		  try{
			  //if the namenode is insafemode,then do not update access time
			  if(isInSafeMode()){
				  doAccessTime = false;
			  }
			  long now = now();
			  INodeFile inode = dir.getFileINode(src);
			  //add by wzt 2013-0328
			  //FSNamesystem.LOG.info("wzt debugfsck:getBlockLocationsInternal inode="+inode.blockReplication
			  //		  +" doAccessTime="+doAccessTime+" supported="+isAccessTimeSupported()+" blocks="+inode.getBlocks());
			  //end add
			  if(inode == null){
			  ...
			  if(doAccessTime && isAccessTimeSupported()){
				  if(now <= inode.getAccessTime() + getAccessTimePrecision()){
					  //if we have to set access time but we only have readlock,then
  ...
  throws IOException {
	 ...
	 try{
		 ...
		 if(blocks == null){
			 //add by wzt 2013-0328
			 //FSNamesystem.LOG.info("wzt debugfsck: getBlockLocationsInternal inode.getBlocks() == null");
			 //end add
			 return null;
		 }
		 if(blocks.length == 0){
		 ...
		 do{
			 ...
			 int numCorruptReplicas = corruptReplicas.numCorruptReplicas(blocks[curBlk]);
			 //add by wzt 2013-0328
			 //FSNamesystem.LOG.info("wzt debugfsck:getBlockLocationsInternal numNodes="+numNodes
			 //		 +" numCorruptNodes="+numCorruptNodes+" numCorruptReplicas="+numCorruptReplicas);
			 if(numCorruptNodes != numCorruptReplicas){
				 LOG.warn("Inconsistent number of corrupt replicas for " + 
			         blocks[curBlk] + "blockMap has " + numCorruptNodes +
			         " but corrupt replicas map has " + numCorruptReplicas);
			 ...
			 if(inode.isUnderConstruction() && curBlk == blocks.length -1
				 ...
				 blockCorrupt = false;
				 //add by wzt 2013-0328
				 //FSNamesystem.LOG.info("wzt debugfsck:debugfsck:getBlockLocationsInternal isUnderConstruction localname="
				//		 +inode.getLocalName());
				 //end add
			 }else{
				 ...
				 if(numMachineSet > 0 ){
					 ...
					 for(Iterator<DatanodeDescriptor> it =
						 ...
						 if(blockCorrupt || (!blockCorrupt && !replicaCorrupt) ){
							 machineSet[numNodes++] = dn;	
							 //add by wzt 2013-0328
							 //FSNamesystem.LOG.info("wzt debugfsck:dn="+dn.name);
							 //end add
						 }
					 }
				 }//end fi
			 ...
			 //add by wzt 2013-0328
			//if(blocks != null){
			//	 blocks[curBlk].getBlockName();
			//	 blocks[curBlk].getBlockId();
			//	 
			// }
			// if(machineSet == null){
			//	 FSNamesystem.LOG.info("wzt debugfsck:machineSet == null"); 
			// }
			// FSNamesystem.LOG.info("wzt debugfsck:blockname="+ blocks[curBlk].getBlockName()+" blockid="+blocks[curBlk].getBlockId());
					 //+" machineSet="+machineSet.length+" curPos="+curPos+" blockCorrupt="+blockCorrupt);
			 //end add
			 results.add(new LocatedBlock(blocks[curBlk],machineSet,curPos,
					 blockCorrupt) );
			 curPos += blocks[curBlk].getNumBytes();
  ...
  private  boolean setReplicationInternal(String src, 
	  ...
	  try{
		  ...
		  int oldRep1 = oldReplication[0];
		  if(oldRep1 == replication){ // the same replication
			  return true;
		  }
		  
		  //update needReplication priority queues
		  for(int idx = 0; idx < fileBlocks.length; idx++){
			  updateNeededReplications(fileBlocks[idx],0,replication - oldRep1);
		  }
		  
		  if(oldRep1 > replication){
			  //old replication  >  the new one;need to remove copies
			  LOG.info("Reducing replication for file "+ src
					  + ". New replication is " + replication);
			  for(int idx = 0; idx < fileBlocks.length; idx++){
  ...
  private void startFileInternal(String src,
	  ...
	  String stateChangeLogDebug = "DIR* NameSystem.startFile: src=" + src
			  +", holder=" + holder
			  +", clientMachine=" + clientMachine
			  +", replication=" + overwrite
			  +", append=" + append;
	  ...
	  try{
		    ...
		    if (pathExists && dir.isDir(src)) {
		     ...
		     //add by wzt 2013-12.13
		     this.fastLogthread.addAuditLog(stateChangeLogDebug);
             //end add
		     ...
		     try {
		      ...
		      if (append) {
		       ...
		       checkFsObjectLimit();
		        ...
		        //add by wzt 2013-12.13
		        String strLog = "DIR* NameSystem.startFile: "+"add "+src+" to namespace for "+holder;
		        this.fastLogthread.addAuditLog(strLog);
		        //end add
		      }
	        } catch (IOException ie) {
	          NameNode.stateChangeLog.warn("DIR* NameSystem.startFile: "
...
//end add
 ...
 // }
  ...
  public LocatedBlock getAdditionalBlock(String src,
	...
	if(targets.length < this.minReplication){
		 throw new IOException("File " + src + " could only be replicated to " + 
	                            targets.length + " nodes, insted of " + 
				                minReplication);
	 }
...
//end update
  ...
  public CompleteFileStatus completeFile(String src, String holder,long index) throws IOException {
    CompleteFileStatus status = completeFileInternal(src, holder,-1,null);
    getEditLog().logSync();
    return status;
  }
  ...
  private CompleteFileStatus completeFileInternal(String src,
	  ...
	  try{
		  ...
		  //add by wzt 2013.12.13
		  String stateChangeLogDebug = "DIR* NameSystem.completeFile: " + src  + " for " + holder+" ...";
		  this.fastLogthread.addAuditLog(stateChangeLogDebug);
		  //end add
		  if(this.isInSafeMode()){
			  throw new SafeModeException("Cannot complete file " + src,safeMode);
		  ...
		  if(inode != null && !inode.isUnderConstruction() && !inode.isDirectory()
			  ...
			  //lease. Then it'll just recommit the edit log and return
			  ...
			  if(sumBlockLen != fileLen){
				 ...
				 String strIOException = "Try close a closed file: file size in client side "
							+ " doesn't match namenode,client: " + fileLen
							+" namenode: " + sumBlockLen;
				 this.fastLogthread.addAuditLog(strIOException); 
			     
				 throw new IOException("Try close a closed file: file size in client side "
						+ " doesn't match namenode,client: " + fileLen
						+" namenode: " + sumBlockLen);  
			  }
			  if(inf.getLastBlock().equals(lastBlock)){
				  throw new IOException("Try close a closed file: last block from client side "
						  + " doesn't match namenode.client: " + lastBlock
						  +" namenode: " + inf.getLastBlock());
			  }
			  ...
			  //		  "DIR* NameSystem.completeFile: process duplicate request: file "
			  //		  + src
			  //		  + " is closed by " 
			  //		  + holder
			  //		  + ((fileLen != -1) ? (" file length " + fileLen) : "")
			  //		  + ((lastBlock != null) ? (" last block: " + lastBlock) : ""));
			  String stateChangeLogInfo ="DIR* NameSystem.completeFile: process duplicate request: file "
					  		  + src
					  		  + " is closed by " 
					  		  + holder
					  		  + ((fileLen != -1) ? (" file length " + fileLen) : "")
					  		  + ((lastBlock != null) ? (" last block: " + lastBlock) : ""+" - CompleteFileStatus.COMPLETE_SUCCESS");
					  
			  this.fastLogthread.addAuditLog(stateChangeLogInfo);
			  return CompleteFileStatus.COMPLETE_SUCCESS;
		  }
		  
		  INodeFileUnderConstruction pendingFile = checkLease(src,holder,inode);
		  ...
		  if(fileBlocks == null){
			  ...
			  //add by wzt 2013-12.13
			  this.fastLogthread.addAuditLog("CompleteFileStatus.STILL_WAITING");
			  //end add
			  return CompleteFileStatus.STILL_WAITING;
		  }
		  ...
		  if(fileLen != -1){
			  ...
			  if(sumBlockLen != fileLen){
				  this.fastLogthread.addAuditLog("file size in client doesn't match the close file.client: " 
						  + fileLen + "namenode: " +sumBlockLen);
				  throw new IOException("file size in client doesn't match the close file.client: " 
						  + fileLen + "namenode: " +sumBlockLen);
			  }
		  }
	  ...
	  String stateChangeLogInfo = null;
	  if(NameNode.stateChangeLog.isDebugEnabled()){
		  //NameNode.stateChangeLog.info("DIR* NameSystem.completeFile: file " + src
		  //		  +" is closed by " + holder
		  //		  + ((fileLen != -1) ? (" file length " + fileLen) : "")
		  //		  + ((lastBlock != null) ? (" last block: " + lastBlock) : "") );
		  stateChangeLogInfo ="DIR* NameSystem.completeFile: file " + src
				  		  +" is closed by " + holder
				  		  + ((fileLen != -1) ? (" file length " + fileLen) : "")
				  		  + ((lastBlock != null) ? (" last block: " + lastBlock) : "") ;
	  }else{
		  //NameNode.stateChangeLog.info("DIR * NameSystem.completeFile: file" + src
		  //		  + " is close by " + holder +" - lease removed");
		  stateChangeLogInfo ="DIR * NameSystem.completeFile: file" + src
				  		  + " is close by " + holder 
				  		 + ((fileLen != -1) ? (" file length " + fileLen) : "")
				  		  + ((lastBlock != null) ? (" last block: " + lastBlock) : "")				  		  
				  		  +" - lease removed  CompleteFileStatus.COMPLETE_SUCCESS";
	  }
	  this.fastLogthread.addAuditLog(stateChangeLogInfo);
	  return CompleteFileStatus.COMPLETE_SUCCESS;
  }
//end add
    ...
    for (int i = 0; i < nrBlocks; i++) {
      ...
      if (number.liveReplicas() < numExpectedReplicas) {
        neededReplications.add(pendingBlocks[i], 
                               ...
                               numExpectedReplicas);
        this.fastLogthread.addAuditLog(" file "+file.getLocalName() 
        		+" block current blocknum "+number.liveReplicas()+" and add neededReplications");
      }
    }
  }
  ...
  private Block allocateBlock(String src, INode[] inodes) throws IOException {
    ...
    //                             +src+ ". "+b);
    String stateChangeLogInfo ="BLOCK* NameSystem.allocateBlock: "
    	                                 +src+ ". "+b;
    this.fastLogthread.addAuditLog(stateChangeLogInfo);
    return b;
  }
  ...
  boolean checkFileProgress(INodeFile v, boolean checkall) {
	  if(checkall){
		  ...
		  //check all blocks of the file.
		  //
		  int closeFileReplicationMin = Math.min(v.getReplication(), this.minReplication);
		  for(Block block : v.getBlocks()){
			  int numNodes = blocksMap.numNodes(block);
			  if(numNodes < closeFileReplicationMin){
				  LOG.info("Closeing INodeFile " + v + " block " + block + " has replication numNodes:"+numNodes
						  +"  closeFileReplicationMin:"+closeFileReplicationMin);
				  return false;
			  }
		  }//end for
	  }else{
...
//end add
  ...
  void dumpExcessReplicasSets(PrintWriter out){
	  ...
	  for(Map.Entry<String, Collection<Block>> entry : excessReplicateMap.entrySet()){
...
//end add
  ...
  public boolean renameTo(String src, String dst) throws IOException {
    boolean status =  renameToInt(src, dst);
    return status;
  }
  
  private boolean renameToInt(String src, String dst) throws IOException {
...
//end add
  ...
  public boolean delete(String src, boolean recursive) throws IOException {
    ...
    boolean status = deleteInt(src, true);
    return status;
  }
  ...
  private boolean  deleteInt(String src, boolean recursive) throws IOException {
	  ...
	  status = deleteInternal(src,recursive);
	  getEditLog().logSync();
	  if (status && auditLog.isInfoEnabled()) {
	     logAuditEvent(UserGroupInformation.getCurrentUGI(),
	                  Server.getRemoteIp(),
...
//end add
  ...
  public boolean mkdirs(String src, PermissionStatus permissions
    ...
    boolean status = mkdirsInt(src, permissions);
    return status;
  }
  ...
  private boolean mkdirsInt(String src, PermissionStatus permissions
	  ...
	  this.writeLock();
	  try{
		  status = mkdirsInternal(src,permissions);
		  if (status && auditLog.isInfoEnabled()){
			  resultingStat = dir.getFileInfo(src);			  
		  }
	  }finally{
...
//end add
  ...
  ContentSummary getContentSummary(String src) throws IOException {
	...
	try{
		if(auditLog.isInfoEnabled()){
			this.logAuditEvent(UserGroupInformation.getCurrentUGI(),
					Server.getRemoteIp(),
					"getContentSummary",src,null,null);
		}
	    return dir.getContentSummary(src);
	}finally{
		this.readUnlock();
...
//end add
  ...
  private void finalizeINodeFileUnderConstruction(String src,
	  ...
	  assert this.hasWriteLock();
	    ...
	    dir.replaceNode(src, pendingFile, newFile);
		  //add by wzt 2013-12.13
		  this.fastLogthread.addAuditLog("finalizeINodeFileUnderConstruction src:"+src+" pendingFileName:"+pendingFile.getLocalName()
				  +" pendingFile lastblock name"+ pendingFile.getLastBlock().getBlockName()
				  +" newFileName:"+newFile.getLocalName()+ " newFile lastblock name:"+newFile.getLastBlock().getBlockName());
		  //end add
	    // close file and persist block allocations for this file
	    dir.closeFile(src, newFile);
	    checkReplicationFactor(newFile);
...
//end add    
  ...
  class ReplicationMonitor implements Runnable {
    ...
    public void run() {
      while (fsRunning) {
        try {
          //add by wzt 2013-0311
          //LOG.debug("wzt rtlock:wl-------FSNamesystem.ReplicationMonitor()----- replicationRecheckInterval = " + replicationRecheckInterval);
          //end add
        	computeDatanodeWork();
          //add by wzt 2013-0311
          //LOG.debug("wzt rtlock:computeDatanodeWork after");
          //end add
          processPendingReplications();
          //add by wzt 2013-0311
          //LOG.debug("wzt rtlock:processPendingReplications after ");
          //end add
  ...
  //    //blocksToProcess = (int)(heartbeats.size() * this.replicationWorkMultiplier);
  //  //add by wl826214---------2012.2.16---start
  ...
  public int computeDatanodeWork() throws IOException {
    ...
    // blocks should not be replicated or removed if safe mode is on
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeDatanodeWork...");
    //end add
    if (isInSafeMode()){
    	return workFound;    	
    }
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeDatanodeWork heartbeats="+heartbeats);
    //end add
    synchronized(heartbeats) {
      blocksToProcess = (int)(heartbeats.size() * this.replicationWorkMultiplier);
      nodesToProcess = (int)Math.ceil((double)heartbeats.size() 
    ...
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:rtlock:computeDatanodeWork synchronized(heartbeats) after ");
    //end add
    workFound = computeReplicationWork(blocksToProcess); 
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeDatanodeWork computeReplicationWork");
    //end add
    // Update FSNamesystemMetrics counters
    this.updateReplicationCounts(workFound);
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeDatanodeWork this.updateReplicationCounts nodesToProcess="+nodesToProcess);
    //end addd
    workFound += computeInvalidateWork(nodesToProcess);    	
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeDatanodeWork computeInvalidateWork workFound="+workFound);
    //end add
    return workFound;
  }
...
private void updateReplicationCounts(int workFound){
	...
	try{
	      ...
	      scheduledReplicationBlocksCount = workFound;
	      corruptReplicaBlocksCount = corruptReplicas.size();
	      //add by wzt 2012-0312
	      //LOG.debug("wzt rtlock:updateReplicationCounts pendingReplicationBlocksCount="+pendingReplicationBlocksCount
	      //		  +" underReplicatedBlocksCount="+underReplicatedBlocksCount
	      //		  +" scheduledReplicationBlocksCount="+scheduledReplicationBlocksCount
	      //		  +" corruptReplicaBlocksCount="+corruptReplicaBlocksCount);
	      //end add
	}finally{
		this.writeUnlock();
	}
...
//end add
  ...
  private int computeInvalidateWork(int nodesToProcess) {
    ...
    for(int nodeCnt = 0; nodeCnt < nodesToProcess; nodeCnt++ ) {
      //add by wzt 2013-0312
      //LOG.debug("wzt rtlock:computeInvalidateWork nodeCnt="+nodeCnt);
      //end add
    ...
    //add by wzt 2013-0312
    //LOG.debug("wzt rtlock:computeInvalidateWork blockCnt="+blockCnt);
    //end add
    return blockCnt;
  }
...
//    		  + excessBlocks.contains(block));
 ...
 private int invalidateWorkForOneNode() {
	 ...
	 //add by wzt 2013-0312
	 //LOG.debug("wzt rtlock:invalidateWorkForOneNode ...");
	 //end add
	 this.writeLock();
	 try{
		 //add by wzt 2013-0312
		 //LOG.debug("wzt rtlock:invalidateWorkForOneNode writeLock after");
		 //end add
		    // blocks should not be replicated or removed if safe mode is on
		    if (isInSafeMode()){
			      return 0;		    	
		    ...
		    //add by wzt 2013-0312
		    //LOG.debug("wzt rtlock:recentInvalidateSets="+recentInvalidateSets);
		    //end add
		    //add by wzt 2013-0312
		    if(recentInvalidateSets == null){
		    	return 0;
		    }
		    //end add
		    if(recentInvalidateSets.isEmpty()){
		    ...
		    String firstNodeId = recentInvalidateSets.keySet().iterator().next();
		    assert firstNodeId != null;
		    dn = datanodeMap.get(firstNodeId);
		    if (dn == null) {
		       removeFromInvalidates(firstNodeId);
		       return 0;
		    }
         ...
         if(NameNode.stateChangeLog.isInfoEnabled() && blocksToInvalidate != null
             ...
             String stateChangeLogInfo ="BLOCK* ask "
                     + dn.getName() + " to delete " + blockList;
         }
	 }
	 return blocksToInvalidate.size();	 
...
//end add
  ...
  void processPendingReplications() {
	  //add by wzt 2013-0312
	  //LOG.debug("wzt rtlock:processPendingReplications... pendingReplications size="+pendingReplications.size());
	  //end add
	  Block[] timedOutItems = pendingReplications.getTimedOutBlocks();
	    if (timedOutItems != null) {
	    	this.writeLock();
	    	try{
	    		//add by wzt 2013-0312
	    		//LOG.debug("wzt rtlock:processPendingReplications  writelock after");
	    		//end add
	    		  for (int i = 0; i < timedOutItems.length; i++) {
	    		       NumberReplicas num = countNodes(timedOutItems[i]);
	    		       neededReplications.add(timedOutItems[i], 
...
//end add  
 ...
 //end add   
  ...
  public void processReport(DatanodeID nodeID, 
	  ...
	  Collection<Block> toAdd = null;
	  Collection<Block> toRemove = null;
	  Collection<Block> toInvalidate = null;
...
//end add
  ...
  Block addStoredBlock(Block block, 
                       DatanodeDescriptor node,
                       DatanodeDescriptor delNodeHint) {
	  return addStoredBlockInternal(block,node,delNodeHint);
  }
  
  private Block  addStoredBlockInternal(Block block,
		                         DatanodeDescriptor node,
		                         DatanodeDescriptor delNodeHint){
	  assert(this.hasWriteLock());
	    BlockInfo storedBlock = blocksMap.getStoredBlock(block);
	    if(storedBlock == null || storedBlock.getINode() == null) {
	      ...
	      //                             + " size " + block.getNumBytes()
	      //                             + " But it does not belong to any file.");
	    String stateChangeLogInfo ="BLOCK* NameSystem.addStoredBlock: "
	  	                                   + "addStoredBlock request received for " 
	  	                                   + block + " on " + node.getName()
	  	                                   + " size " + block.getNumBytes()
	  	                                   + " But it does not belong to any file.";
	    ...
	    if (added) {
	        ...
	        if (!isInSafeMode()) {
	          //NameNode.stateChangeLog.info("BLOCK* NameSystem.addStoredBlock: "
	          //                              +"blockMap updated: "+node.getName()+" is added to "+block+" size "+block.getNumBytes());
	          stateChangeLogInfo ="BLOCK* NameSystem.addStoredBlock: "
	    	                         +"blockMap updated: "+node.getName()+" is added to "+block+" size "+block.getNumBytes();
	          this.fastLogthread.addAuditLog(stateChangeLogInfo);
	        }
	      } else {
...
//update by wzt 2-13-0228
  ...
  //  return nodes;
  //}
  public ArrayList<DatanodeDescriptor> getDatanodeListForReport(
                                                     DatanodeReportType type) {
	  this.readLock();
	  try{
		  boolean listLiveNodes = type == DatanodeReportType.ALL ||

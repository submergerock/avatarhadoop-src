...
public abstract class Storage extends StorageInfo {
 ...
 //Modified by wanglei --2011.11.16
  public void addStorageDir(StorageDirectory sd) {
	  for(StorageDirectory tmp : storageDirs){
		  if(tmp.getCurrentDir().getAbsolutePath().equals(sd.getCurrentDir().getAbsoluteFile()))
				  return;
	  }
    storageDirs.add(sd);
  }
  ...
  //added by wanglei --2011.11.16.01
  public boolean addStorageDirAvatar(StorageDirectory sd) {
	  
	  //LOG.info("add file path " + sd.getCurrentDir().getAbsolutePath());
	  
	  for(StorageDirectory tmp : storageDirs){
		  //LOG.info("current file path " + tmp.getCurrentDir().getAbsolutePath());
		  if(tmp.getCurrentDir().getAbsolutePath().equalsIgnoreCase(sd.getCurrentDir().getAbsolutePath()))
				  return false;
	  }
    storageDirs.add(0, sd);
    return true;
  }
}

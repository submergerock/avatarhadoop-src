...
class JobInProgress {
  ...
  //added by wanglei ----------120911-------start
  private boolean MUSTLOCAL = false;
  //NetworkTopology Node to the set of TIPs
  private int tryTimeUpper;
  private Map<Node, Integer> nodeTimeCache = new HashMap<Node, Integer>();
  //added by wanglei ----------120911-------end
  ...
  public JobInProgress(JobID jobid, JobTracker jobtracker, 
    ...
    LOG.info("default_conf : " + default_conf.getBoolean("map.mustlocal",false));
    LOG.info("conf : " + this.conf.getBoolean("map.mustlocal", false));
    LOG.info("try time upper : " + default_conf.getInt("map.trytime.upper", 3));
    if(default_conf.getBoolean("map.mustlocal",false) == true ||
    		this.conf.getBoolean("map.mustlocal", false) == true){
    	this.MUSTLOCAL = true;
    }
    this.tryTimeUpper = default_conf.getInt("map.trytime.upper", 3);
    LOG.info("this.MUSTLOCAL : "+this.MUSTLOCAL);
  }
  ...
  private Map<Node, List<TaskInProgress>> createCache(
	  ...
	  //LOG.info("-----JobInProgress.createCache()--------");
    ...
    for (int i = 0; i < splits.length; i++) {
      ...
      for(String host: splitLocations) {
        ...
        LOG.info("tip:" + maps[i].getTIPId() + " has split on node:" + node + " host:" + host);
//        if(!maps[i].getSplitNodes().equals(node)){
//        	 LOG.info("");
//        }
  ...
  throws IOException, KillInterruptedException {
    ...
    for(int i=0; i < numMapTasks; ++i) {
      ...
      //LOG.info("splits[i].getLocations()[0]:" + (splits[i].getLocations().length == 0 ? " null " : splits[i].getLocations()[0]));
    }
    LOG.info("Input size for job " + jobId + " = " + inputLength
        + ". Number of splits = " + splits.length);
    if (numMapTasks > 0) { 
      nonRunningMapCache = createCache(splits, maxLevel);
//      Set<Node> n = nonRunningMapCache.keySet();
//      for(Node nod : n){
//      	LOG.info("nod : " + nod);
//      	List <TaskInProgress> tips = nonRunningMapCache.get(nod);
//      	if(tips != null)
//              for(TaskInProgress ti : tips){
//              	LOG.info("ti.getTIPId() : " + ti.getTIPId());
//              }        	
//      }
    }
  ...
  public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, 
	  ...
	  //LOG.info("-----JobInProgress.obtainNewMapTask()--------");
    ...
    //LOG.info("maps[target].getTIPId() : " + maps[target].getTIPId() + " tts.getHost() : " + tts.getHost());
  ...
  public synchronized Task obtainNewLocalMapTask(TaskTrackerStatus tts,
                                                     ...
                                                     int numUniqueHosts)
  throws IOException {
	  //LOG.info("-----JobInProgress.obtainNewLocalMapTask()--------");
    if (!tasksInited.get()) {
      LOG.info("Cannot create task split for " + profile.getJobID());
      return null;
    }
  ...
  public synchronized Task obtainNewNonLocalMapTask(TaskTrackerStatus tts,
                                                    ...
                                                    int numUniqueHosts)
  throws IOException {
	  //LOG.info("-----JobInProgress.obtainNewNonLocalMapTask()--------");
    if (!tasksInited.get()) {
      LOG.info("Cannot create task split for " + profile.getJobID());
      return null;
    }
  ...
  synchronized void addRunningTaskToTIP(TaskInProgress tip, TaskAttemptID id, 
    ...
    // added
	  //LOG.info("-----JobInProgress.addRunningTaskToTIP()--------");
    if (!isScheduled) {
  ...
  private synchronized TaskInProgress findTaskFromList(
	  ...
	  //LOG.info("-----JobInProgress.findTaskFromList()------");
    Iterator<TaskInProgress> iter = tips.iterator();
    while (iter.hasNext()) {
      TaskInProgress tip = iter.next();
      //LOG.info("tip.getSplitLocations()[0]:" + (tip.getSplitLocations().length == 0 ? " null " : tip.getSplitLocations()[0]));
      
      //LOG.info("ttStatus.getHost() = " + ttStatus.getHost());
      //LOG.info("ttStatus.getTrackerName() = " + ttStatus.getTrackerName());
      // Select a tip if
    ...
    while (iter.hasNext()) {
      ...
      if (!tip.hasRunOnMachine(ttStatus.getHost(), 
        ...
        if (tip.hasSpeculativeTask(currentTime, avgProgress)) {
          ...
          //LOG.info("tip.getTIPId().toString():" + tip.getTIPId().toString());
  ...
  private synchronized int findNewMapTask(final TaskTrackerStatus tts, 
	...
	LOG.info("wanglei-----JobInProgress.findNewMapTask()--------");
    if (numMapTasks == 0) {
    ...
    //LOG.info("-----JobInProgress.findNewMapTask()----00000----");
    ...
    // I) Non-running TIP :
    // 
   // LOG.info("-----JobInProgress.findNewMapTask()----11111----");
    ...
    if (node != null) {
      ...
      for (level = 0;level < maxLevelToSchedule; ++level) {
    	//nonRunningMapCache is important : TT <--> List<TaskInProgress>
        List <TaskInProgress> cacheForLevel = nonRunningMapCache.get(key);
        //------------------------------------
        LOG.info("wanglei---node name:" + key.getName()+" networklocation="+key.getNetworkLocation());
        LOG.info("wanglei---tts.getHost() : " + tts.getHost());        	
        
        if(cacheForLevel != null){
        	for(TaskInProgress ti : cacheForLevel){
            	LOG.info("wanglei---ti.getSplitLocations() : " + ti.getSplitLocations());
            }
        }
        
        //add by wanglei 2012.6.1
        if(cacheForLevel == null && MUSTLOCAL == true){
        	 //-------------wl826214---------2011.9.15---------  
        	Integer time = this.nodeTimeCache.get(key);
        	...
        	LOG.info("wanglei---jobID : " + this.jobId + " , Host : " + tts.getHost() +
        			" , tryTime : " + time);
        	
        	
        	this.nodeTimeCache.put(key, time);
        	if(time < this.tryTimeUpper){
        		try{
            		LOG.info("wanglei------------nonRunningMapCache------------");
                    for(Node node0 : nonRunningMapCache.keySet()){
                    	LOG.info("wanglei---node : " + node0);        	
                    	for(TaskInProgress ti : nonRunningMapCache.get(node0)){
                        	LOG.info("wanglei---ti.getSplitLocations() : " + ti.getSplitLocations()[0]);
                        }
                    }
                    
                    LOG.info("wanglei-----------runningMapCache----------");
                    for(Node node0 : this.runningMapCache.keySet()){
                    	LOG.info("wanglei---node : " + node0);        	
                    	for(TaskInProgress ti : runningMapCache.get(node0)){
                        	LOG.info("wanglei---ti.getSplitLocations() : " + ti.getSplitLocations()[0]);
                        }
                    }
                    LOG.info("wanglei----------end---------------");
        			
        		}catch(Exception er){
        			LOG.error("wzt debuginfo:"+er);
        		}
...
//        }
        //---------------------------------------
        LOG.info("wanglei-----JobInProgress.findNewMapTask()----22222----");
        if (cacheForLevel != null) {
          tip = findTaskFromList(cacheForLevel, tts, 
              numUniqueHosts,level == 0);
          LOG.info("wanglei-----JobInProgress.findNewMapTask()----33333----");
          if (tip != null) {
            ...
            LOG.info("wanglei-----JobInProgress.findNewMapTask()----44444----");
      ...
      LOG.info("wanglei-----JobInProgress.findNewMapTask()----55555----");
      // Check if we need to only schedule a local task (node-local/rack-local)
      if (level == maxCacheLevel) {
    	  LOG.info("wanglei-----JobInProgress.findNewMapTask()----66666----");
        return -1;
      }
    }
    LOG.info("wanglei-----JobInProgress.findNewMapTask()----77777----");

...
public class StandbyNew implements Runnable {
  ...
  volatile private IngestLocal ingest;   // object that processes transaction logs from primary
  ...
  //
  StandbyNew(AvatarNodeNew avatarNode, Configuration startupConf, Configuration conf) 
    throws IOException {
    this.running = true;
  ...
  public void run() {
    ...
    while (running) {
      try {
        ...
        // ingestion here.
        if (ingest == null) {
          ingest = new IngestLocal(this, confg, avatarNode.getRemoteEditsFile(startupConf));
  ...
  synchronized void quiesce() throws IOException {
    ...
    if (editnew.exists()) {
      ingest = new IngestLocal(this, confg, editnew);
      ingestThread = new Thread(ingest);
      ingestThread.start(); // start thread to process edits.new

  ...
  class DFSOutputStream extends FSOutputSummer implements Syncable {
          ...
          synchronized (dataQueue) {
            ...
            try {
              ...
              if (blockStream == null) {
                ...
                nodes = nextBlockOutputStream(src); //在这里会初始化blockStream
    ...
    private boolean processDatanodeError(boolean hasError, boolean isAppend) {
      ...
      while (!success && clientRunning) {
        ...
        // any datanodes
        // 
        if (errorIndex < 0) {//通过append()打开文件时，DFSOutputStream构造函数会调用这个方法，这个时候不许要删除任何数据节点
          newnodes = nodes;
        } else {
          if (nodes.length <= 1) {//没有可以使用的数据节点
    ...
    private boolean createBlockOutputStream(DatanodeInfo[] nodes, String client,
      ...
      if (LOG.isDebugEnabled()) {
        for (int i = 0; i < nodes.length; i++) {
          //LOG.debug("pipeline = " + nodes[i].getName()); update by wzt 2013-12.16
      ...
      try {
        //LOG.debug("Connecting to " + nodes[0].getName());//update by wzt 2013-12.16

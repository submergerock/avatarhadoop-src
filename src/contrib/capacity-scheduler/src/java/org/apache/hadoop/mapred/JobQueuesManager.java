...
class JobQueuesManager extends JobInProgressListener {
  ...
  public void jobAdded(JobInProgress job) throws IOException {
    ...
    //scheduler.getOrderedQueues(type)+
    QueueInfo qi = null;
    //QueueInfo qsi = null;
   // Map<String, QueueSchedulingInfo> queueInfoMap = scheduler.getQueueInfoMap();
    String[] s = new String[1];
    if(job.getProfile().getQueueName().equalsIgnoreCase("default")){
    	qi = getQueueInfo(jobQueues,s);
    	job.getProfile().setQueueName(s[0]);
    } else {
    	qi = jobQueues.get(job.getProfile().getQueueName());
    }
    
    
    //job.getProfile().s
    
    //JobProfile jpf = new JobProfile(job.getProfile().getUser(),job.getProfile().getJobID()
    		//,job.getProfile().getJobFile(),job.getProfile().getURL().toString(),);
    
    //QueueInfo qi = jobQueues.get(job.getProfile().getQueueName());
    
    //if(qsi != null && qsi.queueName != null && job.getProfile().getQueueName() != null) {
    //	qi = jobQueues.get(qsi.queueName);
    //} else {
    //	qi = jobQueues.get(job.getProfile().getQueueName());
    //}
    //LOG.info("Job submitted to queue " + qi.);
    if (null == qi) {
  ...
  private QueueInfo getQueueInfo(Map<String, QueueInfo> queueInfoMap,String[] ss){
	  QueueInfo selectedQi = null;
	  
	  QueueInfo qi = null;
	  int numJobs = Integer.MAX_VALUE;
	  int tmp = 0;
	  
	  for(String s : queueInfoMap.keySet()){
		  if(queueInfoMap.isEmpty() != true && queueInfoMap.containsKey(s))
		  qi = queueInfoMap.get(s);
		  tmp = qi.getWaitingJobCount() + qi.getRunningJobCount();
		  LOG.info("qi.queueName:"+ s 
				  +",qsi.getNumRunningAndWaitingJobs():"+tmp);
	  ...
	  LOG.info("===================select a queue!!!======================");
	  LOG.info("qi.queueName:"+ ss[0] 
			  +",qsi.getNumRunningAndWaitingJobs():"+tmp);	
	  LOG.info("===================select a queue!!!======================");

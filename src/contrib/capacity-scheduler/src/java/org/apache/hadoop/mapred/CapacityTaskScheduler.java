...
class CapacityTaskScheduler extends TaskScheduler {
  ...
  public static class QueueSchedulingInfo {
    String queueName;
    ...
    int getNumRunningTasks(){
    	return this.mapTSI.getNumRunningTasks();
    }
    int getWaitingJobs(){
    	return this.jobQueuesManager.getWaitingJobCount(queueName);
    }
    
    int getNumRunningAndWaitingJobs(){
    	int runningJobs = 0;
    	if(this.mapTSI.getNumRunningTasks() != 0){
    		runningJobs = 1;
    	}
    	return this.jobQueuesManager.getWaitingJobCount(queueName) + runningJobs;
    }
  ...
  private static class MapSchedulingMgr extends TaskSchedulingMgr {
    ...
    int getRunningTasks(JobInProgress job) {
    	LOG.info("getRunningTasks() : " + job.runningMaps() );
    ...
    int getSlotsPerTask(JobInProgress job) {
      ...
      int i = (int) (Math.ceil((float) myVmem
              / (float) scheduler.getMemSizeForMapSlot()));
      LOG.info("getSlotsPerTask() : " + i );
      return i;
    }
  ...
  private synchronized void updateQSIObjects(int mapClusterCapacity,
    ...
    for (QueueSchedulingInfo qsi: queueInfoMap.values()) {
      ...
      for (JobInProgress j:
        ...
        LOG.info("=====before=====   qsi.mapTSI.numSlotsOccupied = " + qsi.mapTSI.numSlotsOccupied + "================");
        
        qsi.mapTSI.numSlotsOccupied += numMapSlotsForThisJob;
        
        LOG.info("=====after=====   qsi.mapTSI.numSlotsOccupied = " + qsi.mapTSI.numSlotsOccupied + "================");
        
        LOG.info("==========   numMapSlotsForThisJob = " + numMapSlotsForThisJob + "================");
  ...
  public Map<String, QueueSchedulingInfo> getQueueInfoMap(){

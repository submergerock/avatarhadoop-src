...
public class NetworkTopology {
  ...
  private class InnerNode extends NodeBase {
    ...
    private Node getLoc(String loc) {
    	//LOG.info("-------------------NetworkTopology.getLoc()-----------------");
      if (loc == null || loc.length() == 0) {
    	  //LOG.info("********loc == null || loc.length() == 0*****");
...
//        LOG.info("***************************************************");
//       // if(childnode instanceof DatanodeDescriptor){
//            LOG.info(" children.get("+i+").getName() : " + children.get(i).getName());
//            LOG.info("children.get("+i+").getNetworkLocation() : " + children.get(i).getNetworkLocation());
//       // }       
//        LOG.info("***************************************************");
      }
      //LOG.info("+++++++++++++NetworkTopology.getLoc()+++++++++++++");
   ...
   */
  public Node chooseRandom(String scope,StringBuffer log) {
//	  LOG.info("-----------------------NetworkTopology.chooseRandom()------------------ " );
//	  LOG.info("scope : " + scope );
	  //log.append("NetworkTopology.chooseRandom:01-->");
    netlock.readLock().lock();
    try {
      if (scope.startsWith("~")) {
        return chooseRandom(NodeBase.ROOT, scope.substring(1),log);
      } else {
        return chooseRandom(scope, null,log);
      }
    } finally {
      netlock.readLock().unlock();
    }
  ...
  private Node chooseRandom(String scope, String excludedScope,StringBuffer log){
	  //log.append("NetworkTopology.chooseRandom:02-->");
    ...
    Node node1 = innerNode.getLeaf(leaveIndex, node);
    //log.append("nodeName:" + node1.getName() + "-->");
    return node1;
  }
...
//begin---------add by wanglei---------2011.12.4
   ...
   *          node to be removed
   */ 
  public List<Node> getDatanodeDescriptors(String[] DNIPs) {
	  //LOG.info("-------------------NetworkTopology.getDatanodeDescriptor()-----------------");

	  List<Node> DatanodeDescriptors = new ArrayList<Node>();
	  
	  Node leaf;
	  
	  for(int i = 0 ; i < this.clusterMap.getNumOfLeaves() ; i++){
		  
		  leaf = this.clusterMap.getLeaf(i, null);
		  
		  //LOG.info("leaf["+i+"] : " + leaf);
		  
		  for(String DNIP : DNIPs){
			  if(leaf.toString().split(":")[0].equals(DNIP))								  
					  DatanodeDescriptors.add(leaf);
		  }		  
	  }
	  
//	  LOG.info("DatanodeDescriptors.size() : " + DatanodeDescriptors.size());
//	  for(DatanodeDescriptor dd : DatanodeDescriptors){
//		  LOG.info("dd : " + dd.getHost());
//	  }

...
public class FsShell extends Configured implements Tool {
  ...
  public int run(String argv[]) throws Exception {
    ...
    try {
      ...
      } else if ("-copyFromLocalToDN".equals(cmd)) {
    	  Path src = new Path(argv[i++]);
    	  
    	  String dst = argv[i++];
    	  
    	  String[] DNIPs = new String[argv.length-3];
          for (int j=0 ; i < argv.length ;j++,i++) {
        	  
        	  DNIPs[j] = new String(argv[i]);
        	  
  			  //System.out.println("DNIPs["+j+"] = " + DNIPs[j] );        	  
          }
          copyFromLocalToDN(src, dst,DNIPs);
  ...
  public static void main(String argv[]) throws Exception {
	...
	//System.out.println("------FsShell.main()---111---");
   ...
   * Add local files to the indicated FileSystem name. src is kept.
   */
  void copyFromLocalToDN(Path src, String dstf,String[] DNIPs) throws IOException {
    Path dstPath = new Path(dstf);
    FileSystem dstFs = dstPath.getFileSystem(getConf());
    
  //  if(dstFs instanceof DistributedFileSystem){
    //	DistributedFileSystem dfs = ((DistributedFileSystem)dstFs);
    //	dfs.setDNIPs(DNIPs);
    dstFs.setDNIPs(DNIPs);
    	
//    } else {
//    	try {
//			throw new Exception("FileSystem is not DistributedFileSystem!!");
//		} catch (Exception e) {
...
//    }
    ...
    dstFs.copyFromLocalFile(false, false, src, dstPath);
  }
